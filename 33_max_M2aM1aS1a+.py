...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: M2a-->M1a-->S1a max_plus network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = component_factory(NeuroMLDocument,id="M2aM1aS1a_max_plus")
net = nml_doc.add("Network", id="M2aM1aS1a_max_plus", type="networkWithTemperature",validate=False)
...
def print_separate_connection_stats():
    """Print connection statistics separately for synaptic and electrical connections"""
    # Synaptic connections
    print("\n" + "="*50)
    print("SYNAPTIC CONNECTION STATISTICS")
    print("="*50)
    ...
    if overall_total > 0:
        ...
        print(f"Synaptic connections: {syn_conn} ({syn_conn/overall_total*100:.2f}% of total)")
        print(f"Electrical connections: {ele_conn} ({ele_conn/overall_total*100:.2f}% of total)")
    else:
        print(f"Synaptic connections: {synaptic_contact_stats['total(syn)']} (0.00% of total)")
        print(f"Electrical connections: 0 (0.00% of total)")
...
with ThreadPoolExecutor(max_workers=8) as executor:
    ...
    for cellID, cell_params in modify_cell_files(detailed_cell_files).items():
        for region in M2aM1aS1a_loop.regions:
...
Continuous_Projections = [
    (M2aM1aS1a_loop.regions,PTs,ITs + PTs + TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2aM1aS1a_loop.regions,ITs_c,PTs + CTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2aM1aS1a_loop.regions,ITs_b + ITs_d,PTs + CTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2aM1aS1a_loop.regions,CTs_cSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2aM1aS1a_loop.regions,CTs_bSTUT + CTs_dSTUT,ITs + CTs + CTs + TCs_intralaminar + TCs_matrix, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2aM1aS1a_loop.regions,TCs_intralaminar + TCs_matrix,ITs + TCs_intralaminar + TCs_matrix + PTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M1aS1a_loop.regions,TCs_intralaminar + TCs_core,ITs + TCs_intralaminar + TCs_core + PTs + ITs ,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M1aS1a_loop.regions,CTs_cSTUT,TCs_intralaminar + TCs_core, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M1aS1a_loop.regions,CTs_bSTUT + CTs_dSTUT,TCs_intralaminar + TCs_core, "GABA",Syn_GABAa.id,Syn_GABAb.id),
...
Electrical_Projections = [
    (M2aM1a_loop.regions, TCs_intralaminar + TCs_matrix, TCs_intralaminar + TCs_matrix, "GapJ"),
    (M2aM1a_loop.regions, PTs, PTs, "GapJ"),
    (M2aM1a_loop.regions, CTs, CTs, "GapJ"),
    (M1aS1a_loop.regions, TCs_intralaminar + TCs_core, TCs_intralaminar + TCs_core, "GapJ"),
    (M2aM1aS1a_loop.regions, ITs, ITs, "GapJ")
]
...
InputLists=[
    (M2aM1aS1a_loop.regions, TCs_intralaminar + TCs_matrix,'inh_PG',"GABA"),
    (M1aS1a_loop.regions,TCs_intralaminar + TCs_core,'inh_PG',"GABA"),
    (M2aM1aS1a_loop.regions, CTs_cSTUT,'exc_PG',"AMPA_NMDA"),
    (M2aM1aS1a_loop.regions, CTs_bSTUT + CTs_dSTUT,'inh_PG',"GABA"),
    (M2aM1aS1a_loop.regions, PTs,'exc_PG',"AMPA_NMDA"),
    (M2aM1aS1a_loop.regions, ITs_c,'exc_PG',"AMPA_NMDA"),
    (M2aM1aS1a_loop.regions, ITs_b + ITs_d,'inh_PG',"GABA"),
]
...
def calculate_detailed_connection_statistics():
    ...
    incoming_exc_to_exc = syn_ee # (syn_ee + ele_ee)
    incoming_inh_to_exc = syn_ie # (syn_ie + ele_ie)
    ...
    incoming_exc_to_inh = syn_ei # (syn_ei + ele_ei)
    ...
    from_inh_total = (syn_ie + syn_ii + ele_ii) # + ele_ie 
    inh_to_exc = syn_ie #  (syn_ie + ele_ie)
