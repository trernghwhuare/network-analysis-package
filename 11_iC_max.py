...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: local_intraCortical saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")

nml_doc = component_factory("NeuroMLDocument",id="iC_max")
net = nml_doc.add("Network", id="iC_max", type="networkWithTemperature",validate=False)
...
def classify_cells():
    ...
    exc_cells = set(cell[0] for cell in ITs_c + CTs_cSTUT + PTs)
    inh_cells = set(cell[0] for cell in ITs_b + ITs_d + CTs_bSTUT + CTs_dSTUT)
...
Continuous_Projections = [
    (ITs_c , ITs + CTs + PTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (ITs_b + ITs_d, ITs + CTs + PTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (CTs_cSTUT, ITs + CTs + PTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (CTs_bSTUT + CTs_dSTUT, ITs + CTs + PTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (PTs, ITs + CTs + PTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
]
...
def calculate_connection_statistics():
    ...
    exc_cells = exc_cells_PTs | exc_cells_ITs_c | exc_cells_CTs_c 
    inh_cells = inh_cells_ITs_b | inh_cells_ITs_d | inh_cells_CTs_b | inh_cells_CTs_d 
...
class NeuroMLQuantumOptimizer:
    ...
    def _quantum_connect(self, pre_size, post_size, prob):
        ...
        sparse_indices = cp.random.choice(total, size=int(total*prob), replace=False)
        return cp.unravel_index(sparse_indices, (pre_size, post_size))
...
def create_continuous_projections(network,  prePopList, postPopList, DoubleSynID, preSynID, postSynID):
    ...
    # Initialize GPU with strict memory limits
    cp.cuda.runtime.setDevice(0)
    mempool = cp.get_default_memory_pool()
    mempool.set_limit(size=4 * 1024**3)  # 4GB hard limit
...
def _process_projection_chunk_optimized(chunk, network, DoubleSynID, preSynID, postSynID, MAX_CONNECTIONS, BATCH_SIZE):
    ...
    conn_array = cp.zeros((BATCH_SIZE, MAX_CONNECTIONS), dtype=cp.int32)
    weight_array = cp.zeros((BATCH_SIZE, MAX_CONNECTIONS), dtype=cp.float32)
...
# %%
# Functions to extract and save network data
from collections import defaultdict
from pathlib import Path
import json
import pandas as pd
...
def extract_network_dataset(net, nml_doc):
    ...
    """
    exc_set = _cellid_set_from_tuples(PTs) | _cellid_set_from_tuples(ITs_c) | _cellid_set_from_tuples(CTs_cSTUT) 
    inh_set = _cellid_set_from_tuples(ITs_b) | _cellid_set_from_tuples(ITs_d) | _cellid_set_from_tuples(CTs_bSTUT) | _cellid_set_from_tuples(CTs_dSTUT)
