...
nml_doc = component_factory("NeuroMLDocument",id="TC2IT2PTCT")
net = nml_doc.add("Network", id="TC2IT2PTCT", type="networkWithTemperature",validate=False)
...
def create_population_instances_parallel():
    ...
    all_populations = ITs + PTs + CTs + TCs_matrix + TCs_intralaminar + TCs_core
...
def validate_populations(created_populations):
    ...
    for pop_list in [ITs, PTs, CTs, TCs_matrix + TCs_intralaminar]:
        for pop in pop_list:
...
Continuous_Projections = [
    (TCs_matrix, TCs_matrix + TCs_intralaminar + ITs_b + ITs_d + ITs_c, "AMPA_NMDA", Syn_AMPA.id, Syn_NMDA.id),
    (TCs_intralaminar, TCs_matrix, "GABA", Syn_GABAa.id, Syn_GABAb.id),
    (TCs_intralaminar, TCs_core, "GABA", Syn_GABAa.id, Syn_GABAb.id),
    (TCs_core, TCs_core + TCs_intralaminar + PTs + ITs, "GABA", Syn_GABAa.id, Syn_GABAb.id),
...
def calculate_connection_statistics():
    exc_cells_PTs = set([cell[0] for cell in PTs])
    exc_cells_ITs_c = set([cell[0] for cell in ITs_c])
    exc_cells_CTs_cSTUT = set([cell[0] for cell in CTs_cSTUT])  # Extract cell IDs
    ...
    inh_cells = inh_cells_ITs_d | inh_cells_ITs_b | inh_cells_CTs_dSTUT | inh_cells_CTs_bSTUT | inh_cells_TCs_core | inh_cells_TCs_intralaminar
...
InputLists=[
    ...
    (TCs_core + TCs_intralaminar,'inh_PG',"GABA"),
