...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: layer 2 and 3 neocortical neuronal network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = NeuroMLDocument(id="loop_L23",version="2.0", author="Hua Cheng",description="Layer 2/3 neocortical neurons network with synaptic projections and electrical connections")
net = nml_doc.add("Network", id="loop_L23", type="networkWithTemperature",validate=False)
...
def create_L23_catalog(prefix):
    L23_catalog = []
    detailed_cell_files = sorted(glob.glob(f"cell_files/{prefix}*_L23_*.cell.nml"))
    for cell_file in detailed_cell_files:
        filename_only = os.path.basename(cell_file).replace('.cell.nml', '')
        parts = filename_only.split('_')
        if len(parts) >= 2:
            if parts[1] == "L23":
                L23_catalog.append((filename_only, 36, 2))
    return L23_catalog

IT23_bAC = create_L23_catalog("bAC")
IT23_bIR = create_L23_catalog("bIR")
IT23_bNAC = create_L23_catalog("bNAC")
IT23_cACint = create_L23_catalog("cACint")
IT23_cIR = create_L23_catalog("cIR")
IT23_cNAC = create_L23_catalog("cNAC")
IT23_dNAC = create_L23_catalog("dNAC")

IT23_b = IT23_bAC + IT23_bIR + IT23_bNAC
IT23_c = IT23_cACint + IT23_cIR + IT23_cNAC
IT23_d = IT23_dNAC
IT23 = IT23_b + IT23_c + IT23_d

PT23 = create_L23_catalog("cADpyr")
CT23_cSTUT =  create_L23_catalog("cSTUT")
CT23_dSTUT =  create_L23_catalog("dSTUT")
CT23 = CT23_cSTUT + CT23_dSTUT
...
def classify_cells():
    ...
    exc_cells = set(cell[0] for cell in PT23 + IT23_c + CT23_cSTUT)
    inh_cells = set(cell[0] for cell in IT23_b + IT23_d + CT23_dSTUT)
...
# Base populations
L23_populations = [IT23, PT23, CT23]
# Create individual networks
networks = {}
networks['S1a'] = Network(id="S1a", regions=[regions['S1a']], populations=L23_populations)
networks['S1b'] = Network(id='S1b', regions=[regions['S1b']], populations=L23_populations)
networks['M1a'] = Network(id='M1a', regions=[regions['M1a']], populations=L23_populations)
networks['M1b'] = Network(id='M1b', regions=[regions['M1b']], populations=L23_populations)
networks['M2a'] = Network(id='M2a', regions=[regions['M2a']], populations=L23_populations)
networks['M2b'] = Network(id='M2b', regions=[regions['M2b']], populations=L23_populations)
...
Continuous_Projections = [
    (M2M1S1_loop.regions,PT23,IT23 + PT23,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,IT23_c,IT23 + PT23 + CT23,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,IT23_b,IT23 + PT23 + CT23,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,IT23_d,IT23 + PT23 + CT23,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,CT23_cSTUT,IT23,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,CT23_dSTUT,IT23,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (M2M1S1_loop.regions, IT23, IT23, "GapJ"),
    (M2M1S1_loop.regions, PT23, PT23, "GapJ"),
    (M2M1S1_loop.regions, CT23, CT23, "GapJ"),
]

def calculate_connection_statistics():
    exc_cells_PT23 = set([cell[0] for cell in PT23])  # Extract cell IDs
    exc_cells_IT23_c = set([cell[0] for cell in IT23_c])
    inh_cells_IT23_b = set([cell[0] for cell in IT23_b])
    inh_cells_IT23_d = set([cell[0] for cell in IT23_d])
    exc_cells_CT23_cSTUT = set([cell[0] for cell in CT23_cSTUT])
    inh_cells_CT23_dSTUT = set([cell[0] for cell in CT23_dSTUT])
    exc_cells = exc_cells_PT23 | exc_cells_IT23_c | exc_cells_CT23_cSTUT 
    inh_cells = inh_cells_IT23_b | inh_cells_IT23_d | inh_cells_CT23_dSTUT 
...
InputLists=[
    (M2M1S1_loop.regions,PT23,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,IT23_c,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,IT23_b,'inh_PG',"GABA"),
    (M2M1S1_loop.regions,IT23_d,'inh_PG',"GABA"),
    (M2M1S1_loop.regions,CT23_cSTUT,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,CT23_dSTUT,'inh_PG',"GABA"),
]
...
def extract_network_dataset(net, nml_doc, PT23, IT23, CT23):
    """
    ...
    """
    exc_set = _cellid_set_from_tuples(PT23) | _cellid_set_from_tuples(IT23_c) | _cellid_set_from_tuples(CT23_cSTUT)
    inh_set = _cellid_set_from_tuples(IT23_b) | _cellid_set_from_tuples(IT23_d) | _cellid_set_from_tuples(CT23_dSTUT) 
...
try:
    ...
    basename = getattr(nml_doc, "id", "network")
    dataset = extract_network_dataset(net, nml_doc, PT23, IT23, CT23)
