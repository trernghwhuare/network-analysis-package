...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: M1a<-->M1b network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = component_factory(NeuroMLDocument,id="M1_max_plus")
net = nml_doc.add("Network", id="M1_max_plus", type="networkWithTemperature",validate=False)
...
Continuous_Projections = [
    (M1_loop.regions,PTs,ITs + PTs + TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M1_loop.regions,ITs_c,ITs + PTs + CTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M1_loop.regions,ITs_b + ITs_d,ITs + PTs + CTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M1_loop.regions,CTs_cSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M1_loop.regions,CTs_bSTUT + CTs_dSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M1_loop.regions,CTs_cSTUT,TCs_intralaminar + TCs_core, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M1_loop.regions,CTs_bSTUT + CTs_dSTUT,TCs_intralaminar + TCs_core, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M1_loop.regions,TCs_intralaminar + TCs_matrix,ITs + TCs_intralaminar + TCs_matrix + PTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M1_loop.regions,TCs_intralaminar + TCs_core,ITs + TCs_intralaminar + TCs_core + PTs + ITs ,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (M1_loop.regions, ITs, ITs, "GapJ"),
    (M1_loop.regions, TCs_intralaminar,TCs_intralaminar, "GapJ"),
    (M1_loop.regions, TCs_matrix,TCs_matrix, "GapJ"),
    (M1_loop.regions, TCs_core, TCs_core, "GapJ"),
    (M1_loop.regions, PTs, PTs, "GapJ"),
    (M1_loop.regions, CTs, CTs, "GapJ"),
]
...
InputLists=[
    (M1_loop.regions, ITs_c,'exc_PG',"AMPA_NMDA"),
    (M1_loop.regions, ITs_b + ITs_d,'inh_PG',"GABA"),
    (M1_loop.regions, CTs_cSTUT,'exc_PG',"AMPA_NMDA"),
    (M1_loop.regions, CTs_bSTUT + CTs_dSTUT,'inh_PG',"GABA"),
    (M1_loop.regions, TCs_matrix ,'exc_PG',"AMPA_NMDA"),
    (M1_loop.regions, TCs_core,'inh_PG',"GABA"),
    (M1_loop.regions, TCs_intralaminar,'inh_PG',"GABA"),
    (M1_loop.regions, PTs,'exc_PG',"AMPA_NMDA")
]
# ======================
