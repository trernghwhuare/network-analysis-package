...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: S1a max_plus network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = component_factory(NeuroMLDocument,id="S1a_max_plus")
net = nml_doc.add("Network", id="S1a_max_plus", type="networkWithTemperature",validate=False)
...
def print_separate_connection_stats():
    ...
    print(f"Total synaptic connections: {synaptic_contact_stats['total(syn)']}")
...
with ThreadPoolExecutor(max_workers=8) as executor:
    ...
    for cellID, cell_params in modify_cell_files(detailed_cell_files).items():
        for region in networks['S1a'].regions:
...
Continuous_Projections = [
    (networks['S1a'].regions,PTs,ITs + PTs + TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['S1a'].regions,ITs_c,ITs + PTs + CTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['S1a'].regions,ITs_b + ITs_d,ITs + PTs + CTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['S1a'].regions,CTs_cSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['S1a'].regions,CTs_bSTUT + CTs_dSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['S1a'].regions,CTs_cSTUT,TCs_intralaminar + TCs_core, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['S1a'].regions,CTs_bSTUT + CTs_dSTUT,TCs_intralaminar + TCs_core, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['S1a'].regions,TCs_intralaminar + TCs_matrix,ITs + TCs_intralaminar + TCs_matrix + PTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['S1a'].regions,TCs_intralaminar + TCs_core,ITs + TCs_intralaminar + TCs_core + PTs + ITs ,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (networks['S1a'].regions, TCs_matrix, TCs_matrix, "GapJ"),
    (networks['S1a'].regions, TCs_core, TCs_core, "GapJ"),
    (networks['S1a'].regions, TCs_intralaminar, TCs_intralaminar, "GapJ"),
    (networks['S1a'].regions, ITs, ITs, "GapJ"),
    (networks['S1a'].regions, PTs, PTs, "GapJ"),
    (networks['S1a'].regions, CTs, CTs, "GapJ"),
]
...
InputLists=[
    (networks['S1a'].regions,ITs_c,'exc_PG',"AMPA_NMDA"),
    (networks['S1a'].regions,ITs_b + ITs_c,'inh_PG',"GABA"),
    (networks['S1a'].regions,CTs_cSTUT,'exc_PG',"AMPA_NMDA"),
    (networks['S1a'].regions,CTs_bSTUT + CTs_dSTUT,'inh_PG',"GABA"),
    (networks['S1a'].regions,TCs_intralaminar,'inh_PG',"GABA"),
    (networks['S1a'].regions,TCs_core,'inh_PG',"GABA"),
    (networks['S1a'].regions,TCs_matrix,'exc_PG',"AMPA_NMDA"),
    (networks['S1a'].regions,PTs,'exc_PG',"AMPA_NMDA"),
]
