import random
from pyneuroml.pynml import read_neuroml2_file, validate_neuroml2
...
# Get list of detailed cell files - ONLY LAYER 1 CELLS for this script
prefix_list = ["bNAC", "cACint", "cIR", "cNAC", "cSTUT"]
detailed_cell_files = []
for prefix in prefix_list:
    detailed_cell_files.extend(glob.glob(f"cell_files/{prefix}*_L1_*.cell.nml"))
...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: Layer 1 neocortical neuronal network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = NeuroMLDocument(id="loop_L1", version="2.0", author="Hua Cheng", description="Layer 1 neocortical neurons network with synaptic projections and electrical connections")
net = nml_doc.add("Network", id="loop_L1", type="networkWithTemperature",validate=False)
net.temperature="34.0degC"
net.neuro_lex_id="sao864921383"

# %%

prefix = ["bNAC", "cACint", "cIR", "cNAC", "cSTUT"]
def create_L1_catalog(prefix):
    L1_catalog = []
    detailed_cell_files = sorted(glob.glob(f"cell_files/{prefix}*_L1_*.cell.nml"))
    for cell_file in detailed_cell_files:
        ...
        if len(parts) >= 2:
            if parts[1] == "L1":
                # Store only the filename (without path) as the cell ID to match NeuroML file
                L1_catalog.append((filename_only, 5, 2))
    return L1_catalog

IT1_bNAC = create_L1_catalog("bNAC") 
print(IT1_bNAC)   
IT1_cACint = create_L1_catalog("cACint")
print(IT1_cACint)
IT1_cIR = create_L1_catalog("cIR")
print(IT1_cIR)
IT1_cNAC = create_L1_catalog("cNAC")
print(IT1_cNAC)
CT1_cSTUT = create_L1_catalog("cSTUT")
print(CT1_cSTUT)

IT1_b = IT1_bNAC
IT1_c = IT1_cACint + IT1_cIR + IT1_cNAC
IT1 = IT1_b + IT1_c




if len(IT1) == 0 and len(CT1_cSTUT) == 0:
    print("WARNING: No L1 cells found! Check cell_files directory and naming patterns.")
...
def classify_cells():
    ...
    exc_cells = set(cell[0] for cell in IT1_c + CT1_cSTUT)
    inh_cells = set(cell[0] for cell in IT1_b)
...
# Base populations
L1_pops = [IT1, CT1_cSTUT]
# Create individual networks
networks = {}
networks['S1a'] = Network(id="S1a", regions=[regions['S1a']], populations=L1_pops)
networks['S1b'] = Network(id='S1b', regions=[regions['S1b']], populations=L1_pops)
networks['M1a'] = Network(id='M1a', regions=[regions['M1a']], populations=L1_pops)
networks['M1b'] = Network(id='M1b', regions=[regions['M1b']], populations=L1_pops)
networks['M2a'] = Network(id='M2a', regions=[regions['M2a']], populations=L1_pops)
networks['M2b'] = Network(id='M2b', regions=[regions['M2b']], populations=L1_pops)
...
Continuous_Projections = [
    (M2M1S1_loop.regions,IT1_c,IT1 + CT1_cSTUT,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,IT1_b,IT1 + CT1_cSTUT,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,CT1_cSTUT, IT1,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
]

Electrical_Projections = [
    (M2M1S1_loop.regions, IT1, IT1, "GapJ"),
    (M2M1S1_loop.regions, CT1_cSTUT, CT1_cSTUT, "GapJ"),
]

def calculate_connection_statistics():
    exc_cells_IT1_c = set([cell[0] for cell in IT1_c])
    inh_cells_IT1_b = set([cell[0] for cell in IT1_b])
    exc_cells_CT1_cSTUT = set([cell[0] for cell in CT1_cSTUT])
    exc_cells = exc_cells_IT1_c | exc_cells_CT1_cSTUT 
    inh_cells = inh_cells_IT1_b 
...
InputLists=[
    (M2M1S1_loop.regions,IT1_c,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,IT1_b, 'inh_PG',"GABA"),
    (M2M1S1_loop.regions,CT1_cSTUT,'exc_PG',"AMPA_NMDA"),
]
...
def extract_network_dataset(net, nml_doc, IT1, CT1_cSTUT):
    """
    ...
    """
    exc_set = _cellid_set_from_tuples(IT1_c) | _cellid_set_from_tuples(CT1_cSTUT) 
    inh_set = _cellid_set_from_tuples(IT1_b)  
    ...
    for pop in getattr(net, "populations", []):
        ...
        # e_type, layer, m_type, _ = pop.id.partition('_')
...
try:
    ...
    basename = getattr(nml_doc, "id", "network")
    dataset = extract_network_dataset(net, nml_doc, IT1, CT1_cSTUT)
