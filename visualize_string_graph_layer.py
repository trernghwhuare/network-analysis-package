...
def is_connection_valid_for_network(connection_data, network_name, network_type, regions_list):
    ...
    # Check if this is a multi-region M2 network (all regions start with M2)
    is_multi_m2_network = (
        len(regions_list) > 1 and 
        all(region.startswith('M2') for region in regions_list)
    )
    
    # Apply TCs_core filtering for both pure and multi M2 networks
    if is_pure_m2_network or is_multi_m2_network:
        ...
        # For M2 networks (pure or multi), reject any connections involving TCs_core
