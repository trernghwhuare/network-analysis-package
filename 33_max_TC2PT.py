...
# Define TCs variables before classify_cells function
TCs_intralaminar = [("cell_files/TCRil.cell.nml",2000, 6),("cell_files/nRTil.cell.nml",2000, 6)]
TCs_matrix = [("cell_files/TCR.cell.nml", 6000, 6), ("cell_files/TCRm.cell.nml", 4000, 6),("cell_files/nRTm.cell.nml", 4000, 6)]
TCs_core = [("cell_files/nRT.cell.nml",6000, 6),("cell_files/TCRc.cell.nml", 4000, 6),("cell_files/nRTc.cell.nml", 4000, 6)]
TCs = TCs_matrix + TCs_core + TCs_intralaminar

now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: Thalamus2Cortex saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = component_factory("NeuroMLDocument",id="TC2PT")
net = nml_doc.add("Network", id="TC2PT", type="networkWithTemperature",validate=False)
...
def classify_cells():
    ...
    exc_cells = set(cell[0] for cell in PTs + TCs_matrix)
    inh_cells = set(cell[0] for cell in TCs_intralaminar + TCs_core)
...
def create_population_instances_parallel():
    ...
    all_populations = PTs + TCs_matrix + TCs_intralaminar
...
def validate_populations(created_populations):
    ...
    for pop_list in [PTs, TCs_matrix + TCs_intralaminar]:
        for pop in pop_list:
...
Continuous_Projections = [
    (TCs_matrix, TCs_matrix + TCs_intralaminar + PTs, "AMPA_NMDA", Syn_AMPA.id, Syn_NMDA.id),
    (TCs_intralaminar, TCs_matrix, "GABA", Syn_GABAa.id, Syn_GABAb.id),
    (TCs_intralaminar, TCs_core, "GABA", Syn_GABAa.id, Syn_GABAb.id),
    (TCs_core, TCs_intralaminar + TCs_core + PTs, "GABA", Syn_GABAa.id, Syn_GABAb.id),
    (PTs, PTs, "AMPA_NMDA", Syn_AMPA.id, Syn_NMDA.id)
]
Electrical_Projections = [
    (PTs_L23, PTs_L23, "GapJ"),
    (PTs_L4, PTs_L4, "GapJ"),
    (PTs_L5, PTs_L5, "GapJ"),
    (PTs_L6, PTs_L6, "GapJ"),
...
def calculate_connection_statistics():
    ...
    exc_cells = exc_cells_PTs | exc_cells_TCs_matrix
...
def create_electrical_projections(network, prePopList, postPopList, GapJ_id):
    ...
    global electrical_proj_counter
...
def extract_network_dataset(net, nml_doc):
    ...
    """
    exc_set = _cellid_set_from_tuples(PTs) |  _cellid_set_from_tuples(TCs_matrix)
    ...
    def _get_type(pop_id):
        # First check if population exists in our dataframe
...
print("Network generation complete. Data saved to analysis_out. To run analysis, use: python analysis.py")
