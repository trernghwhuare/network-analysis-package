...
from neuroml import (Population,Location,Instance,IncludeType,Cell,NeuroMLDocument,
                     ElectricalProjection,ElectricalConnectionInstanceW,Property,ContinuousProjection,
                     Input,InputList,PulseGenerator,AlphaSynapse,ExpTwoSynapse,DoubleSynapse,
                     BlockingPlasticSynapse,GapJunction,IncludeType, ContinuousConnectionInstanceW,
                     Population, ElectricalProjection, Instance, Location,Cell,
                     Property,InputW,InputList,PulseGenerator,AlphaSynapse,ExpTwoSynapse,
                     DoubleSynapse,Region,Network,BlockingPlasticSynapse, GapJunction, ElectricalConnectionInstanceW)
from neuroml.utils import component_factory
import neuroml.writers as writers
...
def setup_gpu():
    ...
    return device, pool
gpu_device, gpu_memory_pool = setup_gpu()
...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: IntraThalamic_Conn saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")

nml_doc = component_factory("NeuroMLDocument",id="loop_iT_max_plus")
net = nml_doc.add("Network", id="loop_iT_max_plus", type="networkWithTemperature",validate=False)
net.temperature="34.0degC"
net.neuro_lex_id="sao864921383"
# %%
def get_layer_param(layer_str):
    if 'TCRil' in layer_str or 'nRTil' in layer_str:
        return 2000
    elif 'TCR' in layer_str or 'nRT' in layer_str:
        return 6000
    elif 'TCRc' in layer_str or 'nRTc' in layer_str:
        return 4000
    elif 'TCRm' in layer_str or 'nRTm' in layer_str:
        return 4000  

th_prefix = ["TCR","TCRm","TCRc","TCRil","nRT", "nRTm","nRTc","nRTil"]

def create_thalamus_catalog(prefix):
    """Create catalog for a single cell type prefix"""
    thalamus_catalog = []
    files = sorted(glob.glob(f"cell_files/{prefix}*.cell.nml"))
    for cell_file in files:
        base_name = cell_file.replace('.cell.nml', '')
        layer_param = get_layer_param(base_name)
        thalamus_catalog.append((base_name, layer_param, 6))
    return thalamus_catalog

# Build TCs lists using the updated function
TCs_intralaminar = create_thalamus_catalog("TCRil") + create_thalamus_catalog("nRTil")
TCs_matrix = create_thalamus_catalog("TCR") + create_thalamus_catalog("TCRm") + create_thalamus_catalog("nRTm")
TCs_core = create_thalamus_catalog("nRT") + create_thalamus_catalog("nRTc") + create_thalamus_catalog("TCRc")
TCs = TCs_intralaminar + TCs_matrix + TCs_core

# Create detailed_cell_files by globbing for each prefix
detailed_cell_files = []
for prefix in th_prefix:
    detailed_cell_files.extend(glob.glob(f"cell_files/{prefix}*.cell.nml"))
detailed_cell_files.sort()
detailed_cell_files = sorted(detailed_cell_files)
...
def classify_cells():
    ...
    exc_cells = set(cell[0] for cell in TCs_matrix)
    inh_cells = set(cell[0] for cell in TCs_core + TCs_intralaminar)
...
def update_electrical_conductance_stats(pre_cell_id, post_cell_id, count):
    ...
    with electrical_stats_lock:
        ...
        if pre_is_exc and post_is_exc:
            ...
            electrical_conductance_stats['E-I(ele)'] += count
            persistent_electrical_stats['E-I(ele)'] += count
        elif pre_is_inh and post_is_exc:
            electrical_conductance_stats['I-E(ele)'] += count
            persistent_electrical_stats['I-E(ele)'] += count
        elif pre_is_inh and post_is_inh:
...
base_populations = [TCs_intralaminar, TCs_matrix, TCs_core]
m2_populations = [TCs_intralaminar, TCs_matrix]
networks = {}
...
S1_loops =Network(id="S1_loops",regions=[regions['S1a'],regions['S1b']],populations=[networks['S1a'].populations,networks['S1b'].populations])
M1_loops =Network(id="M1_loops",regions=[regions['M1a'],regions['M1b']],populations=[networks['M1a'].populations,networks['M1b'].populations])
M2_loops= Network(id="M2_loops",regions=[regions['M2a'],regions['M2b']],populations=[networks['M2a'].populations,networks['M2b'].populations])
M1S1_loops =Network(id="M1S1_loops",regions=[regions['S1a'],regions['S1b'],regions['M1a'],regions['M1b']],populations=[networks['S1a'].populations,networks['S1b'].populations,networks['M1a'].populations,networks['M1b'].populations])
M2M1S1_loops = Network(id="M2M1S1_loops",regions=[regions['S1a'],regions['S1b'],regions['M1a'],regions['M1b'],regions['M2a'],regions['M2b']],populations=[networks['S1a'].populations,networks['S1b'].populations,networks['M1a'].populations,networks['M1b'].populations,networks['M2a'].populations,networks['M2b'].populations])
...
def get_layer_id(updated_cellID):
    ...
    raise ValueError(f"Could not extract layer information from {updated_cellID}")
    return None
...
with ThreadPoolExecutor(max_workers=8) as executor:
    ...
    for cellID, cell_params in modify_cell_files(detailed_cell_files).items():
        for region in M2M1S1_loops.regions:
...
Continuous_Projections = [
    (M2_loops.regions,TCs_matrix, TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2_loops.regions,TCs_intralaminar, TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M1S1_loops.regions,TCs_intralaminar, TCs_intralaminar + TCs_matrix + TCs_core,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M1S1_loops.regions,TCs_core, TCs_intralaminar + TCs_core,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loops.regions,TCs_intralaminar, TCs_intralaminar,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (M2_loops.regions, TCs_intralaminar + TCs_matrix, TCs_intralaminar + TCs_matrix, "GapJ"),
    (M1S1_loops.regions, TCs, TCs, "GapJ"),
    (M2M1S1_loops.regions,TCs_intralaminar,TCs_intralaminar, "GapJ"),
]
...
def calculate_connection_statistics():
    ...
    exc_cells = exc_cells_TCs_matrix
    inh_cells = inh_cells_TCs_intralaminar | inh_cells_TCs_core
...
InputLists=[
    (M2_loops.regions,TCs_matrix,'exc_PG',"AMPA_NMDA"),
    (M1S1_loops.regions,TCs_core,'inh_PG',"GABA"),
    (M1S1_loops.regions,TCs_matrix,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loops.regions,TCs_intralaminar,'inh_PG',"GABA"),
]
...
def create_quantum_pulse_generators(nml_doc,rng_seed=42):
    rng = cp.random.default_rng(seed=rng_seed)  # Quantum-inspired reproducible randomness
    base_configs = {
...
def create_quantum_input_lists(nml_doc, pulse_generators, rng_seed=42):
    ...
    for regions, PopLists, PG_id, dest in tqdm(InputLists, desc="Quantum input mapping"):
        for region in regions:
            RegionID = region.id
            for cellID, pop_size, axonalNum in PopLists:
                Pop_id = f"{RegionID}_{cellID}"
                for PG_id in ['inh_PG', 'exc_PG']:
                    ...
                    if pg_id not in pulse_generators:
                        continue
                    # if PG_id.startswith('inh_PG'):
                    #     if inh_count >= MAX_INH:
                    #         continue
                    #     ratio = INH_RATIO
                    #     max_count = MAX_INH - inh_count
                    #     count_ref = 'inh_count'
                    # else:
                    #     if exc_count >= MAX_EXC:
                    #         continue
                    #     ratio = EXC_RATIO
                    #     max_count = MAX_EXC - exc_count
                    #     count_ref = 'exc_count'
                    # n_connections = min(int(pop_size * ratio), max_count)
                    ...
                    for i in range(0, len(connected_cells), BATCH_SIZE):
                        batch_cells = connected_cells[i:i+BATCH_SIZE]
                        inputs = [
                            InputW(
                                id=j+i,
                                target=f"../{Pop_id}/{cell_idx}/{cellID}",
                                destination=dest,
                                weight=f"{random.uniform(0.1, 0.5):.2f}"
                            )
                            for j, cell_idx in enumerate(batch_cells)
                        ]
                        input_list.input.extend(inputs)
                    net.input_lists.append(input_list)
                    # if PG_id.startswith('inh'):
                    #     inh_count += 1
                    # else:
                    #     exc_count += 1
...
def extract_network_dataset(net, nml_doc):
    ...
    """
    exc_set =  _cellid_set_from_tuples(TCs_matrix)
    inh_set =  _cellid_set_from_tuples(TCs_intralaminar) | _cellid_set_from_tuples(TCs_core)
