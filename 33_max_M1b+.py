...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: M1b max_plus network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = component_factory(NeuroMLDocument,id="M1b_max_plus")
net = nml_doc.add("Network", id="M1b_max_plus", type="networkWithTemperature",validate=False)
...
with ThreadPoolExecutor(max_workers=8) as executor:
    ...
    for cellID, cell_params in modify_cell_files(detailed_cell_files).items():
        for region in networks['M1b'].regions:
...
Continuous_Projections = [
    (networks['M1b'].regions,PTs,ITs + PTs + TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M1b'].regions,ITs_c,ITs + PTs + CTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M1b'].regions,ITs_b + ITs_d,ITs + PTs + CTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['M1b'].regions,CTs_cSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M1b'].regions,CTs_bSTUT + CTs_dSTUT,ITs + CTs +TCs_intralaminar + TCs_matrix, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['M1b'].regions,CTs_cSTUT,TCs_intralaminar + TCs_core, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M1b'].regions,CTs_bSTUT + CTs_dSTUT,TCs_intralaminar + TCs_core, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['M1b'].regions,TCs_intralaminar + TCs_matrix,ITs + TCs_intralaminar + TCs_matrix + PTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['M1b'].regions,TCs_intralaminar + TCs_core,ITs + TCs_intralaminar + TCs_core + PTs + ITs ,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (networks['M1b'].regions, TCs_matrix, TCs_matrix, "GapJ"),
    (networks['M1b'].regions, TCs_core, TCs_core, "GapJ"),
    (networks['M1b'].regions, TCs_intralaminar, TCs_intralaminar, "GapJ"),
    (networks['M1b'].regions, ITs, ITs, "GapJ"),
    (networks['M1b'].regions, PTs, PTs, "GapJ"),
    (networks['M1b'].regions, CTs, CTs, "GapJ"),
]
...
InputLists=[
    (networks['M1b'].regions,ITs_c,'exc_PG',"AMPA_NMDA"),
    (networks['M1b'].regions,ITs_b + ITs_d,'inh_TPF',"GABA"),
    (networks['M1b'].regions,CTs_cSTUT,'exc_PG',"AMPA_NMDA"),
    (networks['M1b'].regions,CTs_bSTUT + CTs_dSTUT,'inh_TPF',"GABA"),
    (networks['M1b'].regions,TCs_intralaminar + TCs_matrix ,'exc_PG',"AMPA_NMDA"),
    (networks['M1b'].regions,TCs_intralaminar + TCs_core,'inh_TPF',"GABA"),
    (networks['M1b'].regions,PTs,'exc_PG',"AMPA_NMDA")
]
