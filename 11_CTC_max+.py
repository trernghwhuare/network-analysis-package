...
from neuroml import (ContinuousConnectionInstanceW,ContinuousProjection, ElectricalProjection,
                     Instance,Location,Population,NeuroMLDocument,Property,InputW,InputList,IncludeType,
                     PulseGenerator,ExpTwoSynapse,DoubleSynapse,BlockingPlasticSynapse, GapJunction, 
...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: cortex -> thalamus -> cortex network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = NeuroMLDocument(id="max_CTC_plus")
net = nml_doc.add("Network", id="max_CTC_plus", type="networkWithTemperature",validate=False)
...
# %%
def create_population_instances(cell_params, start_index, lock):
    ...
    specific_pop.add(Property, tag="space", value=f'{layer_id}')
...
Continuous_Projections = [
    (PTs, ITs + PTs + TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (ITs_c, ITs + PTs + CTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (ITs_b + ITs_d,ITs + PTs + CTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (CTs_cSTUT, ITs + CTs + TCs_intralaminar + TCs_matrix + TCs_core, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (CTs_bSTUT + CTs_dSTUT, ITs + CTs + TCs_intralaminar + TCs_matrix + TCs_core, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (TCs_matrix, TCs + PTs + CTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (TCs_intralaminar, TCs_matrix + TCs_core, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (TCs_core, TCs + PTs + CTs ,"GABA", Syn_GABAa.id,Syn_GABAb.id)
]
