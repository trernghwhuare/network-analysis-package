...
from neuroml import (IncludeType, ContinuousConnectionInstanceW,ContinuousProjection, Population, 
                     ...
                     PulseGenerator,AlphaSynapse,ExpTwoSynapse,DoubleSynapse,BlockingPlasticSynapse, TransientPoissonFiringSynapse,
...
nml_doc = component_factory("NeuroMLDocument",id="TC2IT4_IT2CT")
net = nml_doc.add("Network", id="TC2IT4_IT2CT", type="networkWithTemperature",validate=False)
...
# Dynamically generate CT cell lists
...
def classify_cells():
    ...
    exc_cells = set(cell[0] for cell in IT4_c + ITs_c + CTs_cSTUT + TCs_matrix)
    inh_cells = set(cell[0] for cell in IT4_b + IT4_d + ITs_b + ITs_d + CTs_bSTUT +  CTs_dSTUT + TCs_core + TCs_intralaminar)
...
def create_population_instances_parallel():
    ...
    all_populations = IT4 + ITs + CTs + TCs_intralaminar + TCs_core + TCs_matrix
...
def validate_populations(created_populations):
    ...
    for pop_list in [IT4, ITs, CTs, TCs_intralaminar + TCs_core]:
        for pop in pop_list:
...
Continuous_Projections = [
    (TCs_intralaminar, TCs_intralaminar + TCs_core + TCs_matrix, "GABA", Syn_GABAa.id, Syn_GABAb.id),
    (TCs_matrix, TCs_intralaminar + TCs_matrix + IT4, "GABA", Syn_GABAa.id, Syn_GABAb.id),
    (IT4_c, ITs + CTs, "AMPA_NMDA", Syn_AMPA.id, Syn_NMDA.id),
    (IT4_b + IT4_d, ITs + CTs, "GABA", Syn_GABAa.id, Syn_GABAb.id),
    (ITs_c, ITs + CTs, "AMPA_NMDA", Syn_AMPA.id, Syn_NMDA.id),
    (ITs_b + ITs_d, ITs + CTs, "GABA", Syn_GABAa.id, Syn_GABAb.id),
    (CTs_cSTUT, CTs + TCs_core, "AMPA_NMDA", Syn_AMPA.id, Syn_NMDA.id),
    (CTs_bSTUT + CTs_dSTUT, CTs + TCs_core, "GABA", Syn_GABAa.id, Syn_GABAb.id),
    (TCs_core, TCs_intralaminar + CTs, "GABA", Syn_GABAa.id, Syn_GABAb.id),
]
...
Electrical_Projections = [
    ...
    (ITs_b, ITs_b, "GapJ"),
    (ITs_d, ITs_d, "GapJ"),
    (CTs_cSTUT, CTs_cSTUT, "GapJ"),
    (CTs_bSTUT, CTs_bSTUT, "GapJ"),
    (CTs_dSTUT, CTs_dSTUT, "GapJ"),
]
...
def calculate_connection_statistics():
    ...
    exc_cells = exc_cells_IT4_c | exc_cells_ITs_c | exc_cells_CTs_cSTUT | exc_cells_TCs_matrix
    inh_cells = inh_cells_IT4_b | inh_cells_IT4_d | inh_cells_ITs_b | inh_cells_ITs_d | inh_cells_CTs_bSTUT | inh_cells_CTs_dSTUT | inh_cells_TCs_core | inh_cells_TCs_intralaminar
...
InputLists=[
    ...
    (CTs_bSTUT, 'inh_PG',"GABA"),
    (CTs_dSTUT, 'inh_PG',"GABA"),
]
...
def extract_network_dataset(net, nml_doc):
    ...
    """
    exc_set = _cellid_set_from_tuples(IT4_c) | _cellid_set_from_tuples(ITs_c) | _cellid_set_from_tuples(CTs_cSTUT) | _cellid_set_from_tuples(TCs_matrix)
    inh_set = _cellid_set_from_tuples(IT4_b) | _cellid_set_from_tuples(IT4_d) | _cellid_set_from_tuples(ITs_b) | _cellid_set_from_tuples(ITs_d) | _cellid_set_from_tuples(CTs_bSTUT) | _cellid_set_from_tuples(CTs_dSTUT) | _cellid_set_from_tuples(TCs_intralaminar) | _cellid_set_from_tuples(TCs_core) 
