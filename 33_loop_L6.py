...
from neuroml import (ContinuousConnectionInstanceW,ContinuousProjection, ElectricalProjection, 
                     Instance,IonChannel,Location, Space, Region, Cell,Population,
                     NeuroMLDocument,Property,InputW,InputList,IncludeType,
                     PulseGenerator,AlphaSynapse,ExpTwoSynapse,DoubleSynapse,
                     BlockingPlasticSynapse, GapJunction,Region, Network,
...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: minimun layer 6 neocortical neuronal network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = NeuroMLDocument(id="loop_L6", version="2.0", author="Hua Cheng",description="Layer 6 neocortical neurons network with synaptic projections and electrical connections")
net = nml_doc.add("Network", id="loop_L6", type="networkWithTemperature",validate=False)
net.temperature="34.0degC"
net.neuro_lex_id="sao864921383"

# %%
prefix = ["bNAC", "bIR", "bNAC", "cACint", "cIR", "cNAC", "dNAC",
          "cADpyr", "bSTUT", "cSTUT", "dSTUT"]

def create_L6_catalog(prefix):
    L6_catalog = []
    detailed_cell_files = sorted(glob.glob(f"cell_files/{prefix}*_L6_*.cell.nml"))
    for cell_file in detailed_cell_files:
        filename_only = os.path.basename(cell_file).replace('.cell.nml', '')
        parts = filename_only.split('_')
        if len(parts) >= 2:
            if parts[1] == "L6":
                L6_catalog.append((filename_only, 52, 2))
    return L6_catalog

IT6_bAC = create_L6_catalog("bAC")
IT6_bIR = create_L6_catalog("bIR")
IT6_bNAC = create_L6_catalog("bNAC")
IT6_cACint = create_L6_catalog("cACint")
IT6_cIR = create_L6_catalog("cIR")
IT6_cNAC = create_L6_catalog("cNAC")
IT6_dNAC = create_L6_catalog("dNAC")
IT6_b = IT6_bAC + IT6_bIR + IT6_bNAC
IT6_c = IT6_cACint + IT6_cIR + IT6_cNAC
IT6_d = IT6_dNAC
IT6 = IT6_b + IT6_c + IT6_d

PT6 = create_L6_catalog("cADpyr")

CT6_bSTUT =  create_L6_catalog("bSTUT")
CT6_cSTUT =  create_L6_catalog("cSTUT")
CT6_dSTUT =  create_L6_catalog("dSTUT")
CT6 = CT6_bSTUT + CT6_cSTUT + CT6_dSTUT
...
def classify_cells():
    ...
    exc_cells = set(cell[0] for cell in PT6 + IT6_c + CT6_cSTUT)
    inh_cells = set(cell[0] for cell in IT6_b + IT6_d + CT6_bSTUT + CT6_dSTUT)
    return exc_cells, inh_cells


EXC_CELLS, INH_CELLS = classify_cells()

def update_synaptic_connection_stats(pre_cell_id, post_cell_id, count):
    """Update synaptic connection statistics in a thread-safe manner"""
...
def update_electrical_connection_stats(pre_cell_id, post_cell_id, count):
    """Update electrical connection statistics in a thread-safe manner"""
...
def print_separate_connection_stats():
    ...
    print("\n" + "="*50)
    print("ELECTRICAL CONNECTION STATISTICS")
    print("="*50)
    ...
    if persistent_electrical_stats['total(ele)'] > 0:
        ...
        print(f"Total electrical connections: {pers_total}")
    ...
    if ele_total > 0:
        ...
        print(f"Total electrical connections: {ele_total}")
    else:
        print("WARNING: No electrical connections were created or statistics not updated")
...
# Base populations
L6_populations = [IT6, PT6, CT6]
# Create individual networks
networks = {}
networks['S1a'] = Network(id="S1a", regions=[regions['S1a']], populations=L6_populations)
networks['S1b'] = Network(id='S1b', regions=[regions['S1b']], populations=L6_populations)
networks['M1a'] = Network(id='M1a', regions=[regions['M1a']], populations=L6_populations)
networks['M1b'] = Network(id='M1b', regions=[regions['M1b']], populations=L6_populations)
networks['M2a'] = Network(id='M2a', regions=[regions['M2a']], populations=L6_populations)
networks['M2b'] = Network(id='M2b', regions=[regions['M2b']], populations=L6_populations)

# Create loop networks
M2M1S1_loop = Network(id="M2M1S1",
                      regions=[regions['M2a'], regions['M1a'], regions['S1a'], regions['S1b'], regions['M1b'], regions['M2b']],
                      populations=[networks['S1a'].populations, networks['S1b'].populations, networks['M1a'].populations,
                                   networks['M1b'].populations, networks['M2a'].populations, networks['M2b'].populations])
...
def get_layer_id(updated_cellID):
    if 'nRT' in updated_cellID or 'nRTc' in updated_cellID:
        return "TCs_core"
    elif 'TCR' in updated_cellID or 'TCRm' in updated_cellID:
        return "TCs_matrix"
...
created_populations = {}
# net = nml_doc.add("Network", id="max_M2M1S1_plus", type="networkWithTemperature",validate=False)
...
Continuous_Projections = [
    (M2M1S1_loop.regions,PT6,IT6 + PT6,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,IT6_c,IT6 + PT6 + CT6,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,IT6_b,IT6 + PT6 + CT6,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,IT6_d,IT6 + PT6 + CT6,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,CT6_cSTUT,IT6,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,CT6_bSTUT,IT6,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,CT6_dSTUT,IT6,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (M2M1S1_loop.regions, IT6, IT6, "GapJ"),
    (M2M1S1_loop.regions, PT6, PT6, "GapJ"),
    (M2M1S1_loop.regions, CT6, CT6, "GapJ"),
]
def calculate_connection_statistics():
    exc_cells_PT6 = set([cell[0] for cell in PT6])  # Extract cell IDs
    exc_cells_IT6_c = set([cell[0] for cell in IT6_c])
    inh_cells_IT6_b = set([cell[0] for cell in IT6_b])
    inh_cells_IT6_d = set([cell[0] for cell in IT6_d])
    exc_cells_CT6_cSTUT = set([cell[0] for cell in CT6_cSTUT])
    inh_cells_CT6_bSTUT = set([cell[0] for cell in CT6_bSTUT])
    inh_cells_CT6_dSTUT = set([cell[0] for cell in CT6_dSTUT])
    exc_cells = exc_cells_PT6 | exc_cells_IT6_c | exc_cells_CT6_cSTUT 
    inh_cells = inh_cells_IT6_b | inh_cells_IT6_d | inh_cells_CT6_bSTUT | inh_cells_CT6_dSTUT 
...
InputLists=[
    (M2M1S1_loop.regions,PT6,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,IT6_c,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,IT6_b,'inh_PG',"GABA"),
    (M2M1S1_loop.regions,IT6_d,'inh_PG',"GABA"),
    (M2M1S1_loop.regions,CT6_cSTUT,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,CT6_bSTUT,'inh_PG',"GABA"),
    (M2M1S1_loop.regions,CT6_dSTUT,'inh_PG',"GABA"),
]
    ...
    for i in range(0, len(chunk), BATCH_SIZE):
        ...
        for j, (pre_region_id, post_region_id, pre_cellID, pre_size, pre_axonalNum,
            ...
            try:
                ...
                # conn_prob = min(0.01, np.sqrt(10/(pre_size * post_size)))
                # size = min(pre_size, post_size, MAX_CONNECTIONS)
                # Adjusted connectivity probability based on cell type classification
                ...
                # update_actual_connection_stats(pre_cellID, post_cellID, n_connections)
                update_synaptic_connection_stats(pre_cellID, post_cellID, n_connections)
            except Exception as e:
        ...
        for pre_region, post_region, pre_cell, post_cell in valid_combinations:
            ...
            try:
                ...
                if len(conn_indices[0]) > 0:
                    ...
                    pre_segments = np.random.randint(pre_axonalNum, size=num_connections)
                    post_segments = np.random.randint(post_axonalNum, size=num_connections)
                    ...
                    proj_batch.append(elec_proj)

                    update_electrical_connection_stats(pre_cellID, post_cellID, num_connections)
                    # if num_connections > 0:
                        # print(f"DEBUG: Created {num_connections} electrical connections between {pre_cellID} and {post_cellID}")
                        # check_electrical_stats()
                # Add projections to network in batches
                if len(proj_batch) >= PROJ_BATCH_SIZE:
                    network.electrical_projections.extend(proj_batch)
                    proj_batch = []
                    gc.collect()
                unique_counter += 1
            except Exception as e:
        ...
        for region in regions:
            ...
            for cellID, pop_size, axonalNum in PopLists:
                ...
                for pg_type in  ('inh_PG', 'exc_PG'):
                    ...
                    # # decide counts based on type
                    # if pg_type == 'inh_PG':
                    #     if inh_count >= MAX_INH:
                    #         continue
                    #     n_connections = max(1, int(pop_size * INH_RATIO))
                    # else:
                    #     if exc_count >= MAX_EXC:
                    #         continue
                    #     n_connections = max(1, int(pop_size * EXC_RATIO))
                    ...
                    connected_cells = np.random.choice(pop_size, size=min(n_connections, pop_size), replace=False)
...
def extract_network_dataset(net, nml_doc, PT6, IT6, CT6):
    """
    ...
    """
    exc_set = _cellid_set_from_tuples(PT6) | _cellid_set_from_tuples(IT6_c) | _cellid_set_from_tuples(CT6_cSTUT)
    inh_set = _cellid_set_from_tuples(IT6_b) | _cellid_set_from_tuples(IT6_d) | _cellid_set_from_tuples(CT6_bSTUT) | _cellid_set_from_tuples(CT6_dSTUT)
...
try:
    ...
    basename = getattr(nml_doc, "id", "network")
    dataset = extract_network_dataset(net, nml_doc, PT6, IT6, CT6)
