...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: layer 5 neocortical neuronal network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = NeuroMLDocument(id="loop_L5", version="2.0", author="Hua Cheng",description="Layer 5 neocortical neurons network with synaptic projections and electrical connections")
net = nml_doc.add("Network", id="loop_L5", type="networkWithTemperature",validate=False)
...
def create_L5_catalog(prefix):
    L5_catalog = []
    detailed_cell_files = sorted(glob.glob(f"cell_files/{prefix}*_L5_*.cell.nml"))
    for cell_file in detailed_cell_files:
        filename_only = os.path.basename(cell_file).replace('.cell.nml', '')
        parts = filename_only.split('_')
        if len(parts) >= 2:
            if parts[1] == "L5":
                L5_catalog.append((filename_only, 25, 2))
    return L5_catalog

IT5_bAC = create_L5_catalog("bNAC")
IT5_bIR = create_L5_catalog("bIR")
IT5_bNAC = create_L5_catalog("bNAC")
IT5_cACint = create_L5_catalog("cACint")
IT5_cIR = create_L5_catalog("cIR")
IT5_cNAC = create_L5_catalog("cNAC")
IT5_dNAC = create_L5_catalog("dNAC")

IT5_b = IT5_bAC + IT5_bIR + IT5_bNAC
IT5_c = IT5_cACint + IT5_cIR + IT5_cNAC
IT5_d = IT5_dNAC
IT5 = IT5_b + IT5_c + IT5_d

PT5 = create_L5_catalog("cADpyr")

CT5_bSTUT =  create_L5_catalog("bSTUT")
CT5_cSTUT =  create_L5_catalog("cSTUT")
CT5_dSTUT =  create_L5_catalog("dSTUT")
CT5 = CT5_bSTUT + CT5_cSTUT + CT5_dSTUT 
...
def classify_cells():
    ...
    exc_cells = set(cell[0] for cell in PT5 + IT5_c + CT5_cSTUT)
    inh_cells = set(cell[0] for cell in IT5_b + IT5_d + CT5_bSTUT + CT5_dSTUT)
...
# Base populations
L5_populations = [IT5, PT5, CT5]
# Create individual networks
networks = {}
networks['S1a'] = Network(id="S1a", regions=[regions['S1a']], populations=L5_populations)
networks['S1b'] = Network(id='S1b', regions=[regions['S1b']], populations=L5_populations)
networks['M1a'] = Network(id='M1a', regions=[regions['M1a']], populations=L5_populations)
networks['M1b'] = Network(id='M1b', regions=[regions['M1b']], populations=L5_populations)
networks['M2a'] = Network(id='M2a', regions=[regions['M2a']], populations=L5_populations)
networks['M2b'] = Network(id='M2b', regions=[regions['M2b']], populations=L5_populations)
...
Continuous_Projections = [
    (M2M1S1_loop.regions,PT5,IT5 + PT5,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,IT5_c,IT5 + PT5 + CT5,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,IT5_b,IT5 + PT5 + CT5,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,IT5_d,IT5 + PT5 + CT5,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,CT5_cSTUT,IT5,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,CT5_bSTUT,IT5,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,CT5_dSTUT,IT5,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (M2M1S1_loop.regions, IT5, IT5, "GapJ"),
    (M2M1S1_loop.regions, PT5, PT5, "GapJ"),
    (M2M1S1_loop.regions, CT5, CT5, "GapJ"),
]

def calculate_connection_statistics():
    exc_cells_PT5 = set([cell[0] for cell in PT5])  # Extract cell IDs
    exc_cells_IT5_c = set([cell[0] for cell in IT5_c])
    inh_cells_IT5_b = set([cell[0] for cell in IT5_b])
    inh_cells_IT5_d = set([cell[0] for cell in IT5_d])
    exc_cells_CT5_cSTUT = set([cell[0] for cell in CT5_cSTUT])
    inh_cells_CT5_bSTUT = set([cell[0] for cell in CT5_bSTUT])
    inh_cells_CT5_dSTUT = set([cell[0] for cell in CT5_dSTUT])
    exc_cells = exc_cells_PT5 | exc_cells_IT5_c | exc_cells_CT5_cSTUT 
    inh_cells = inh_cells_IT5_b | inh_cells_IT5_d | inh_cells_CT5_bSTUT | inh_cells_CT5_dSTUT 
...
InputLists=[
    (M2M1S1_loop.regions,PT5,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,IT5_c,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,IT5_b,'inh_PG',"GABA"),
    (M2M1S1_loop.regions,IT5_d,'inh_PG',"GABA"),
    (M2M1S1_loop.regions,CT5_cSTUT,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,CT5_bSTUT,'inh_PG',"GABA"),
    (M2M1S1_loop.regions,CT5_dSTUT,'inh_PG',"GABA"),
]
...
def extract_network_dataset(net, nml_doc, PT5, IT5, CT5):
    """
    ...
    """
    exc_set = _cellid_set_from_tuples(PT5) | _cellid_set_from_tuples(IT5_c) | _cellid_set_from_tuples(CT5_cSTUT)
    inh_set = _cellid_set_from_tuples(IT5_b) | _cellid_set_from_tuples(IT5_d) | _cellid_set_from_tuples(CT5_bSTUT) | _cellid_set_from_tuples(CT5_dSTUT)
...
try:
    ...
    basename = getattr(nml_doc, "id", "network")
    dataset = extract_network_dataset(net, nml_doc, PT5, IT5, CT5)
