...
nml_doc = component_factory("NeuroMLDocument",id="T2C_max_plus")
net = nml_doc.add("Network", id="T2C_max_plus", type="networkWithTemperature",validate=False)
...
def create_population_instances_parallel():
    ...
    all_populations = ITs + PTs + CTs + TCs_matrix + TCs_core + TCs_intralaminar
...
def validate_populations(created_populations):
    ...
    for pop_list in [ITs, PTs, CTs, TCs_matrix + TCs_core + TCs_intralaminar]:
        for pop in pop_list:
...
Continuous_Projections = [
    (TCs_matrix, TCs_matrix + TCs_intralaminar + PTs + CTs ,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (TCs_core, TCs_core + TCs_intralaminar + PTs + CTs ,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (TCs_intralaminar, TCs_matrix + TCs_core, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (PTs, ITs, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (ITs_c, ITs + CTs, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (ITs_b + ITs_d, ITs + CTs, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (CTs_cSTUT, CTs + ITs, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (CTs_bSTUT + CTs_dSTUT, CTs + ITs, "GABA",Syn_GABAa.id,Syn_GABAb.id),
]
