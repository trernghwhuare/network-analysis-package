...
def plot_combined_heatmaps(pop_stats_df, cols=None, outpath="analysis_out/combined_heatmaps.png", 
                          methods=["spearman", "pearson"], basename=None):
    """
    ...
    for i, config in enumerate(plot_configs):
        ...
        if "E/I" in config["title"] and "Ratio" in config["title"]:
            ...
            if "exc_inputs" in df.columns and "inh_inputs" in df.columns:
                ...
                ax.scatter(df[x_col], ei_ratio, alpha=0.7, s=100, 
            ...
            if region_colors:
                for region in unique_regions:
                    ...
                    ax.scatter(region_data[x_col], region_data[y_col], 
                              alpha=0.7, s=100, label=region, 
                              color=region_colors[region], edgecolors='black', linewidth=0.5)
            else:
...
def find_available_datasets(indir="analysis_out"):
    ...
    Returns:
      list of basenames found in the directory
    """
...
def process_multiple_datasets(indir="analysis_out", outdir="analysis_out"):
    """
    ...
    for basename in basenames:
        try:
            ...
            # Generate scatter plots (now includes all three comparisons in one plot)
            res = plot_ei_scatter_with_stacked(
                dataset["pop_stats_df"], 
...
def main():
    ...
    parser.add_argument("--outdir", "-o", default="analysis_out", help="Output directory for plots")
    ...
    if args.all or not args.basename:
        ...
        print("Saved:", res["outpath"])
        print("Regressions:", res["regressions"])
        
        ei_stats = aggregate_EI_stats(dataset["pop_stats_df"])
        print("E/I aggregated stats:", ei_stats)
