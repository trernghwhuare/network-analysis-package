...
from neuroml import (IncludeType, ContinuousConnectionInstanceW,ContinuousProjection, Population, 
                     ...
                     PulseGenerator,TransientPoissonFiringSynapse, AlphaSynapse,ExpTwoSynapse,DoubleSynapse,BlockingPlasticSynapse, 
...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: M1a max_plus network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = component_factory(NeuroMLDocument,id="M1a_max_plus")
net = nml_doc.add("Network", id="M1a_max_plus", type="networkWithTemperature",validate=False)
...
with ThreadPoolExecutor(max_workers=8) as executor:
    ...
    for cellID, cell_params in modify_cell_files(detailed_cell_files).items():
        for region in networks['M1a'].regions:
...
Continuous_Projections = [
    (networks['M1a'].regions,PTs,ITs + PTs + TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M1a'].regions,ITs_c,ITs + PTs + CTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M1a'].regions,ITs_b + ITs_d,ITs + PTs + CTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['M1a'].regions,CTs_cSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M1a'].regions,CTs_bSTUT + CTs_dSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['M1a'].regions,CTs_cSTUT,TCs_intralaminar + TCs_core, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M1a'].regions,CTs_bSTUT + CTs_dSTUT,TCs_intralaminar + TCs_core, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['M1a'].regions,TCs_intralaminar + TCs_matrix,ITs + TCs_intralaminar + TCs_matrix + PTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M1a'].regions,TCs_intralaminar + TCs_core,ITs + TCs_intralaminar + TCs_core + PTs + ITs ,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (networks['M1a'].regions, TCs_intralaminar + TCs_matrix, TCs_intralaminar + TCs_matrix, "GapJ"),
    (networks['M1a'].regions, TCs_intralaminar + TCs_core, TCs_intralaminar + TCs_core, "GapJ"),
    (networks['M1a'].regions, ITs, ITs, "GapJ"),
    (networks['M1a'].regions, PTs, PTs, "GapJ"),
    (networks['M1a'].regions, CTs, CTs, "GapJ"),
]
...
InputLists=[
    (networks['M1a'].regions,ITs_c,'exc_PG',"AMPA_NMDA"),
    (networks['M1a'].regions,ITs_b + ITs_d,'inh_TPF',"GABA"),
    (networks['M1a'].regions,CTs_cSTUT,'exc_PG',"AMPA_NMDA"),
    (networks['M1a'].regions,CTs_bSTUT + CTs_dSTUT,'inh_TPF',"GABA"),
    (networks['M1a'].regions,TCs_intralaminar + TCs_matrix ,'exc_PG',"AMPA_NMDA"),
    (networks['M1a'].regions,TCs_intralaminar + TCs_core,'inh_TPF',"GABA"),
    (networks['M1a'].regions,PTs,'exc_PG',"AMPA_NMDA")
]
...
# ======================
# Quantum-Inspired pulse_generators
# ======================
        ...
        for region in regions:
            ...
            for cellID, pop_size, axonalNum in PopLists:
                ...
                for pg_type in  ('inh_PG', 'exc_PG'):
                    ...
                    ratio = INH_RATIO if PG_id.startswith('inh_TPF') else EXC_RATIO
...
# ======================
rng_seed = 42
# Create pulse_generators (called once)
...
def extract_network_dataset(net, nml_doc):
    ...
    pg_list = getattr(nml_doc, "pulse_generators", []) or []
    exc_pg = sum(1 for pg in pg_list if getattr(pg, "id", "").startswith("exc"))
    inh_pg = sum(1 for pg in pg_list if getattr(pg, "id", "").startswith("inh"))
    ...
    summary = {
        ...
        "pulse_generators": {"exc": exc_pg, "inh": inh_pg},
