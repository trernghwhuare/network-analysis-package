...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: M2<-->M2 max_plus network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = component_factory(NeuroMLDocument,id="M2_max_plus")
net = nml_doc.add("Network", id="M2_max_plus", type="networkWithTemperature",validate=False)
...
TCs_intralaminar = [("TCRil",200, 6),("nRTil",200, 6)]
TCs_matrix = [("TCR", 600, 6), ("TCRm", 400, 6),("nRTm", 400, 6)]
TCs_core = [("nRT",600, 6),("TCRc", 400, 6),("nRTc", 400, 6)]
...
def update_synaptic_contact_stats(pre_cellID, post_cellID, count):
...
def update_electrical_conductance_stats(pre_cellID, post_cellID, count):
...
def get_layer_id(updated_cellID):
    ...
    #     return "TCs_core"
    if 'TCR' in updated_cellID or 'TCRm' in updated_cellID or 'nRTm' in updated_cellID:
        return "TCs_matrix"
    elif 'TCRil' in updated_cellID or 'nRTil' in updated_cellID:
        return "TCs_intralaminar"
    for layer in ['L1', 'L23', 'L4', 'L5', 'L6', 'TCs_intralaminar', 'TCs_matrix']:  # 'TCs_core' removed
...
Continuous_Projections = [
    (M2_loop.regions,PTs,ITs + PTs + TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2_loop.regions,ITs_c,ITs + PTs + CTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2_loop.regions,ITs_b + ITs_d,ITs + PTs + CTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2_loop.regions,CTs_cSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2_loop.regions,CTs_bSTUT + CTs_dSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2_loop.regions,TCs_intralaminar + TCs_matrix,ITs + TCs_intralaminar + TCs_matrix + PTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (M2_loop.regions,TCs_intralaminar + TCs_matrix, TCs_intralaminar + TCs_matrix, "GapJ"),
    (M2_loop.regions,PTs, PTs, "GapJ"),
    (M2_loop.regions,CTs, CTs, "GapJ"),
    (M2_loop.regions,ITs, ITs, "GapJ")
]
...
def calculate_connection_statistics():
    ...
    inh_cells = inh_cells_ITs_b | inh_cells_ITs_d | inh_cells_CTs_bSTUT | inh_cells_CTs_dSTUT | inh_cells_TCs_intralaminar
    ...
    for regions, prePopList, postPopList, syn_type, *syn_ids in Continuous_Projections:
        for pre_cell in prePopList:
            for post_cell in postPopList:
                pre_cellID = pre_cell[0]
                post_cellID = post_cell[0]
...
InputLists=[
    (M2_loop.regions,TCs_intralaminar + TCs_matrix,'inh_PG',"GABA"),
    (M2_loop.regions,CTs_cSTUT,'exc_PG',"AMPA_NMDA"),
    (M2_loop.regions,CTs_bSTUT + CTs_dSTUT,'inh_PG',"GABA"),
    (M2_loop.regions,PTs,'exc_PG',"AMPA_NMDA"),
    (M2_loop.regions,ITs_c,'exc_PG',"AMPA_NMDA"),
    (M2_loop.regions,ITs_b + ITs_d,'inh_PG',"GABA"),
]
...
def extract_network_dataset(net, nml_doc):
    ...
    inh_set = _cellid_set_from_tuples(ITs_b) | _cellid_set_from_tuples(ITs_d) | _cellid_set_from_tuples(CTs_bSTUT) | _cellid_set_from_tuples(CTs_dSTUT) | _cellid_set_from_tuples(TCs_intralaminar)
