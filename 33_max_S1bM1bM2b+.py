...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: S1b-->M1b-->M2b max_plus network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = component_factory(NeuroMLDocument,id="S1bM1bM2b_max_plus")
net = nml_doc.add("Network", id="S1bM1bM2b_max_plus", type="networkWithTemperature",validate=False)
...
def classify_cells():
    ...
    inh_cells = set(cell[0] for cell in ITs_b + ITs_d + CTs_bSTUT + CTs_dSTUT + TCs_intralaminar)
...
with ThreadPoolExecutor(max_workers=8) as executor:
    ...
    for cellID, cell_params in modify_cell_files(detailed_cell_files).items():
        for region in S1bM1bM2b_loop.regions:
...
Continuous_Projections = [
    ...
    (S1bM1bM2b_loop.regions,PTs,ITs + PTs + TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (S1bM1bM2b_loop.regions,ITs_c, PTs + CTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (S1bM1bM2b_loop.regions,ITs_b + ITs_d, PTs + CTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (S1bM1bM2b_loop.regions,CTs_cSTUT, ITs + CTs + TCs_intralaminar + TCs_matrix, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (S1bM1bM2b_loop.regions,CTs_bSTUT + CTs_dSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (S1bM1bM2b_loop.regions,TCs_intralaminar + TCs_matrix,ITs + TCs_intralaminar + TCs_matrix + PTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
...
Electrical_Projections = [
    (S1bM1b_loop.regions, TCs_intralaminar + TCs_core, TCs_intralaminar + TCs_core, "GapJ"),
    (S1bM1bM2b_loop.regions, TCs_intralaminar + TCs_matrix, TCs_intralaminar + TCs_matrix, "GapJ"),
    (S1bM1bM2b_loop.regions, PTs, PTs, "GapJ"),
    (S1bM1bM2b_loop.regions, CTs, CTs, "GapJ"),
    (S1bM1bM2b_loop.regions, ITs, ITs, "GapJ")
]
...
InputLists=[
    (S1bM1b_loop.regions, TCs_intralaminar + TCs_core,'inh_PG',"GABA"),
    (S1bM1bM2b_loop.regions, TCs_intralaminar + TCs_matrix,'inh_PG',"GABA"),
    (S1bM1bM2b_loop.regions, CTs_cSTUT,'exc_PG',"AMPA_NMDA"),
    (S1bM1bM2b_loop.regions, CTs_bSTUT + CTs_dSTUT,'inh_PG',"GABA"),
    (S1bM1bM2b_loop.regions, PTs,'exc_PG',"AMPA_NMDA"),
    (S1bM1bM2b_loop.regions, ITs_c,'exc_PG',"AMPA_NMDA"),
    (S1bM1bM2b_loop.regions, ITs_b,'inh_PG',"GABA"),
    (S1bM1bM2b_loop.regions, ITs_d,'inh_PG',"GABA"),
]
...
def extract_network_dataset(net, nml_doc, PTs, ITs, CTs, TCs):
    """
    ...
    inh_set = _cellid_set_from_tuples(ITs_b) | _cellid_set_from_tuples(ITs_d) | _cellid_set_from_tuples(CTs_bSTUT) | _cellid_set_from_tuples(CTs_dSTUT) | _cellid_set_from_tuples(TCs_core) | _cellid_set_from_tuples(TCs_intralaminar)
...
try:
    ...
    basename = getattr(nml_doc, "id", "network")
    dataset = extract_network_dataset(net, nml_doc, PTs, ITs, CTs, TCs)
