...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: M2b max_plus network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = component_factory(NeuroMLDocument,id="M2b_max_plus")
net = nml_doc.add("Network", id="M2b_max_plus", type="networkWithTemperature",validate=False)
...
with ThreadPoolExecutor(max_workers=8) as executor:
    ...
    for cellID, cell_params in modify_cell_files(detailed_cell_files).items():
        for region in networks['M2b'].regions:
...
Continuous_Projections = [
    (networks['M2b'].regions,PTs,ITs + PTs + TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M2b'].regions,ITs_c,ITs + PTs + CTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M2b'].regions,ITs_b + ITs_d,ITs + PTs + CTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['M2b'].regions,CTs_cSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (networks['M2b'].regions,CTs_bSTUT + CTs_dSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (networks['M2b'].regions,TCs_intralaminar + TCs_matrix,ITs + TCs_intralaminar + TCs_matrix + PTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (networks['M2b'].regions, TCs_intralaminar + TCs_matrix, TCs_intralaminar + TCs_matrix, "GapJ"),
    (networks['M2b'].regions, ITs, ITs, "GapJ"),
    (networks['M2b'].regions, PTs, PTs, "GapJ"),
    (networks['M2b'].regions, CTs, CTs, "GapJ"),
]
...
InputLists=[
    (networks['M2b'].regions,TCs_intralaminar + TCs_matrix,'inh_PG',"GABA"),
    (networks['M2b'].regions,ITs_c,'exc_PG',"AMPA_NMDA"),
    (networks['M2b'].regions,ITs_b + ITs_d,'inh_PG',"GABA"),
    (networks['M2b'].regions,CTs_cSTUT,'exc_PG',"AMPA_NMDA"),
    (networks['M2b'].regions,CTs_bSTUT + CTs_dSTUT,'inh_PG',"GABA"),
    (networks['M2b'].regions,PTs,'exc_PG',"AMPA_NMDA"),
]
