...
def setup_gpu():
    ...
    pool = np.cuda.MemoryPool()
    np.cuda.set_allocator(pool.malloc)
    # Pre-warm GPU
    _ = np.random.uniform(size=(1000, 1000))
    return device, pool
...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: S1a<-->S1b max_plus network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = component_factory(NeuroMLDocument,id="S1_max_plus")
net = nml_doc.add("Network", id="S1_max_plus", type="networkWithTemperature",validate=False)
...
def classify_cells():
    ...
    inh_cells = set(cell[0] for cell in ITs_b + ITs_d + CTs_bSTUT + CTs_dSTUT + TCs_core + TCs_intralaminar)
...
Continuous_Projections = [
    (S1_loop.regions,PTs,ITs + PTs + TCs_intralaminar + TCs_matrix,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (S1_loop.regions,ITs_c,ITs + PTs + CTs,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (S1_loop.regions,ITs_b + ITs_d,ITs + PTs + CTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (S1_loop.regions,CTs_cSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (S1_loop.regions,CTs_bSTUT + CTs_dSTUT,ITs + CTs + TCs_intralaminar + TCs_matrix, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (S1_loop.regions,CTs_cSTUT,TCs_intralaminar + TCs_core, "AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (S1_loop.regions,CTs_bSTUT + CTs_dSTUT,TCs_intralaminar + TCs_core, "GABA",Syn_GABAa.id,Syn_GABAb.id),
    (S1_loop.regions,TCs_intralaminar + TCs_matrix,ITs + TCs_intralaminar + TCs_matrix + PTs,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (S1_loop.regions,TCs_intralaminar + TCs_core,ITs + TCs_intralaminar + TCs_core + PTs + ITs ,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (S1_loop.regions, TCs_intralaminar,TCs_intralaminar, "GapJ"),
    (S1_loop.regions, TCs_matrix,TCs_matrix, "GapJ"),
    (S1_loop.regions, TCs_core, TCs_core, "GapJ"),
    (S1_loop.regions, PTs, PTs, "GapJ"),
    (S1_loop.regions, CTs, CTs, "GapJ"),
    (S1_loop.regions, ITs, ITs, "GapJ")
]
...
def calculate_connection_statistics():
    ...
    inh_cells = inh_cells_ITs_b | inh_cells_ITs_d | inh_cells_CTs_bSTUT | inh_cells_CTs_dSTUT | inh_cells_TCs_core | inh_cells_TCs_intralaminar
...
InputLists=[
    (S1_loop.regions, ITs_c,'exc_PG',"AMPA_NMDA"),
    (S1_loop.regions, ITs_b + ITs_d,'inh_PG',"GABA"),
    (S1_loop.regions, CTs_cSTUT,'exc_PG',"AMPA_NMDA"),
    (S1_loop.regions, CTs_bSTUT + CTs_dSTUT,'inh_PG',"GABA"),
    (S1_loop.regions, TCs_intralaminar + TCs_matrix ,'exc_PG',"AMPA_NMDA"),
    (S1_loop.regions, TCs_intralaminar + TCs_core,'inh_PG',"GABA"),
    (S1_loop.regions, PTs,'exc_PG',"AMPA_NMDA")
]
...
def calculate_input_statistics(nml_doc):
    ...
    if len(valid_exc_counts) > 1 and len(valid_inh_counts) > 1:
        ...
        try:
            # Try to use cupy array get() method
...
def export_connection_statistics(syn_ee, syn_ei, syn_ie, syn_ii, ele_ee, ele_ii, correlation):
...
def calculate_detailed_connection_statistics():
    ...
    export_connection_statistics(syn_ee, syn_ei, syn_ie, syn_ii, ele_ee, ele_ii, stats['pearson_r'])
