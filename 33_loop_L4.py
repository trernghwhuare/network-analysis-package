...
now = datetime.now()
print(f"Network structure (NeuroML 2beta5) for project: Layer 4 neocortical neuronal network saved with pyNeuroML on {now.strftime('%H:%M:%S %Y-%m-%d')}")
nml_doc = NeuroMLDocument(id="loop_L4", version="2.0", author="Hua Cheng", description="Layer 4 neocortical neuronal network with synaptic projections and electrical connections")
net = nml_doc.add("Network", id="loop_L4", type="networkWithTemperature",validate=False)
net.temperature="34.0degC"
net.neuro_lex_id="sao864921383"

# %%

prefix = ["bAC", "bIR", "bNAC", "cACint", "cIR", "cNAC", "dNAC", 
          "cADpyr","bSTUT", "cSTUT", "dSTUT"]

def create_L4_catalog(prefix):
    L4_catalog = []
    detailed_cell_files = sorted(glob.glob(f"cell_files/{prefix}*_L4_*.cell.nml"))
    for cell_file in detailed_cell_files:
        # base_name = cell_file.replace('.cell.nml', '')
        filename_only = os.path.basename(cell_file).replace('.cell.nml', '')
        parts = filename_only.split('_')
        # parts = base_name.split('_')
        if len(parts) >= 2:
            if parts[1] == "L4":
                L4_catalog.append((filename_only, 21, 2))
    return L4_catalog

IT4_bAC = create_L4_catalog("bAC")
IT4_bIR = create_L4_catalog("bIR")
IT4_bNAC = create_L4_catalog("bNAC")
IT4_cACint = create_L4_catalog("cACint")
IT4_cIR = create_L4_catalog("cIR")
IT4_cNAC = create_L4_catalog("cNAC")
IT4_dNAC = create_L4_catalog("dNAC")

IT4_b = IT4_bAC + IT4_bIR + IT4_bNAC
IT4_c = IT4_cACint + IT4_cIR + IT4_cNAC
IT4_d = IT4_dNAC
IT4 = IT4_b + IT4_c + IT4_d

PT4 = create_L4_catalog("cADpyr")

CT4_bSTUT =  create_L4_catalog("bSTUT")
CT4_cSTUT =  create_L4_catalog("cSTUT")
CT4_dSTUT =  create_L4_catalog("dSTUT")

CT4 = CT4_bSTUT + CT4_cSTUT + CT4_dSTUT
...
def classify_cells():
    ...
    exc_cells = set(cell[0] for cell in PT4 + IT4_c + CT4_cSTUT)
    inh_cells = set(cell[0] for cell in IT4_b + IT4_d + CT4_bSTUT + CT4_dSTUT)
...
# Base populations
L4_populations = [IT4, PT4, CT4]
# Create individual networks
networks = {}
networks['S1a'] = Network(id="S1a", regions=[regions['S1a']], populations=L4_populations)
networks['S1b'] = Network(id='S1b', regions=[regions['S1b']], populations=L4_populations)
networks['M1a'] = Network(id='M1a', regions=[regions['M1a']], populations=L4_populations)
networks['M1b'] = Network(id='M1b', regions=[regions['M1b']], populations=L4_populations)
networks['M2a'] = Network(id='M2a', regions=[regions['M2a']], populations=L4_populations)
networks['M2b'] = Network(id='M2b', regions=[regions['M2b']], populations=L4_populations)
...
Continuous_Projections = [
    (M2M1S1_loop.regions,PT4,IT4 + PT4,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,IT4_c,IT4 + PT4 + CT4,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,IT4_b,IT4 + PT4 + CT4,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,IT4_d,IT4 + PT4 + CT4,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,CT4_cSTUT,IT4,"AMPA_NMDA",Syn_AMPA.id,Syn_NMDA.id),
    (M2M1S1_loop.regions,CT4_bSTUT,IT4,"GABA",Syn_GABAa.id,Syn_GABAb.id),
    (M2M1S1_loop.regions,CT4_dSTUT,IT4,"GABA",Syn_GABAa.id,Syn_GABAb.id),
]

Electrical_Projections = [
    (M2M1S1_loop.regions, IT4, IT4, "GapJ"),
    (M2M1S1_loop.regions, PT4, PT4, "GapJ"),
    (M2M1S1_loop.regions, CT4, CT4, "GapJ"),
]

def calculate_connection_statistics():
    exc_cells_PT4 = set([cell[0] for cell in PT4])  # Extract cell IDs
    exc_cells_IT4_c = set([cell[0] for cell in IT4_c])
    inh_cells_IT4_b = set([cell[0] for cell in IT4_b])
    inh_cells_IT4_d = set([cell[0] for cell in IT4_d])
    exc_cells_CT4_cSTUT = set([cell[0] for cell in CT4_cSTUT])
    inh_cells_CT4_bSTUT = set([cell[0] for cell in CT4_bSTUT])
    inh_cells_CT4_dSTUT = set([cell[0] for cell in CT4_dSTUT])
    exc_cells = exc_cells_PT4 | exc_cells_IT4_c | exc_cells_CT4_cSTUT 
    inh_cells = inh_cells_IT4_b | inh_cells_IT4_d | inh_cells_CT4_bSTUT | inh_cells_CT4_dSTUT 
...
InputLists=[
    (M2M1S1_loop.regions,PT4,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,IT4_c,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,IT4_b,'inh_PG',"GABA"),
    (M2M1S1_loop.regions,IT4_d,'inh_PG',"GABA"),
    (M2M1S1_loop.regions,CT4_cSTUT,'exc_PG',"AMPA_NMDA"),
    (M2M1S1_loop.regions,CT4_bSTUT,'inh_PG',"GABA"),
    (M2M1S1_loop.regions,CT4_dSTUT,'inh_PG',"GABA"),
]
...
def extract_network_dataset(net, nml_doc, PT4, IT4, CT4):
    """
    ...
    """
    exc_set = _cellid_set_from_tuples(PT4) | _cellid_set_from_tuples(IT4_c) | _cellid_set_from_tuples(CT4_cSTUT)
    inh_set = _cellid_set_from_tuples(IT4_b) | _cellid_set_from_tuples(IT4_d) | _cellid_set_from_tuples(CT4_bSTUT) | _cellid_set_from_tuples(CT4_dSTUT)
...
try:
    ...
    basename = getattr(nml_doc, "id", "network")
    dataset = extract_network_dataset(net, nml_doc, PT4, IT4, CT4)
